...
class OrderCreationActions:
    ...
    def find_cells_for_request(
        ...
        try:
            ...
            if status != "PENDING":
                # Заявка уже в другом состоянии — не трогаем
    ...
    def create_order_from_request(
        ...
        - обновляет locker_cells: ставит status='locker_reserved', current_order_id = order_id;
        - помечает заявку COMPLETED и привязывает order_id.

        Возвращает:
        - success: bool
        - order_id: Optional[int]
        - error_code: str ('', 'ORDER_REQUEST_NOT_FOUND', 'INVALID_REQUEST_STATE')
        """
        session = self.session
        try:
            # 1. Читаем заявку
            ...
            if status != "PENDING":
                # Кто-то уже пометил заявку как FAILED/COMPLETED
            ...
            # 3. Создаём заказ (одной транзакцией вместе с резервом ячеек и обновлением заявки)
            # Вставка в orders
            session.execute(
                text(
                    """
                ...
                {
                    "desc": description,
                    "from_city": "LOCAL",   # при необходимости поменяешь на реальные города
                    "to_city": "LOCAL",
            ...
            # 4. Резервируем ячейки под заказ
            session.execute(
                text(
                    """
                    UPDATE locker_cells
