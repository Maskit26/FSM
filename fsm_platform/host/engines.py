...
def register_domain_engine(service_id: str, url: str, **engine_kwargs: object) -> Engine:
    ...
    return _register_engine_pair(
        _engine_by_service_id,
        _sessionmaker_by_service_id,
        service_id,
        url,
        **engine_kwargs,
    )
...
def graph_session(service_id: str) -> Session:
    """Сессия к domain DB для чтения FSM-графа (DOMAIN_GRAPH_DATABASE_URL / db_graph_secret_ref)."""
    if service_id not in _graph_read_sessionmaker_by_service_id:
        raise RuntimeError(
            ...
            f"set DOMAIN_GRAPH_DATABASE_URL or domain_services.db_graph_secret_ref"
        )
    return _graph_read_sessionmaker_by_service_id[service_id]()


def graph_write_session(service_id: str) -> Session:
    """Сессия для graph/publish (DOMAIN_GRAPH_WRITE_DATABASE_URL / db_graph_write_secret_ref)."""
    if service_id not in _graph_write_sessionmaker_by_service_id:
        raise RuntimeError(
            ...
            f"set DOMAIN_GRAPH_WRITE_DATABASE_URL or domain_services.db_graph_write_secret_ref"
        )
    return _graph_write_sessionmaker_by_service_id[service_id]()


def has_graph_read_engine(service_id: str) -> bool:
    return service_id in _graph_read_engine_by_service_id


def has_graph_write_engine(service_id: str) -> bool:
    return service_id in _graph_write_engine_by_service_id


def clear_engines() -> None:
    """Сбрасывает все engines (для тестов). Закрывает пулы соединений."""
    global _platform_engine, _platform_sessionmaker
    for eng in set(_engine_by_url.values()):
        eng.dispose()
    _engine_by_url.clear()
    _sessionmaker_by_url.clear()
    _engine_by_service_id.clear()
    _sessionmaker_by_service_id.clear()
    _graph_read_engine_by_service_id.clear()
    _graph_read_sessionmaker_by_service_id.clear()
    _graph_write_engine_by_service_id.clear()
    _graph_write_sessionmaker_by_service_id.clear()
