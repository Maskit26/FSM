...
_LOCKER_ACCESS_CODE_RULES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "user_role": "driver",
        "stage_must_be": "driver_reserved",
        # Водитель едет в другой город — city mismatch не режем.
        "require_city": False,
        "require_cell": True,
        "require_pin": False,
        "allowed_statuses": [
...
def can_complete_trip(
    ...
    """
    complete_trip: driver + trip_in_progress + все заказы
    order_parcel_confirmed_post2 + нет открытых dest-ячеек.
    """
    _ = db
    _ = session_domain
    _ = instance
    ...
    required_status = params.get("required_status") or "trip_in_progress"
    status = str(trip.get("status") or "")
    if status != str(required_status):
        return GuardResult(ok=False, reason=f"INVALID_TRIP_STATUS:{status}")
    ...
    open_cells = ctx.get("open_delivery_cells") or []
    if open_cells:
        return GuardResult(ok=False, reason=f"OPEN_CELLS_DETECTED:{open_cells}")

    pending = ctx.get("undelivered_order_ids") or []
    if pending:
        return GuardResult(
            ok=False, reason=f"ORDERS_NOT_DELIVERED:{pending}"
        )
    return GuardResult(ok=True)
